...
import { SegTreeViz } from './SegTreeViz'
...
export function DataStructureVisualizer({ algorithm }: Props) {
  const [seed, setSeed] = useState(0)
  const [codeCollapsed, setCodeCollapsed] = useState(false)
  const steps = useMemo(() => algorithm.generate() as any[], [algorithm, seed])
  const player = useStepPlayer(steps, 400)
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-hidden">
            {current && algorithm.kind === 'stack' && <StackViz step={current} />}
            {current && algorithm.kind === 'queue' && <QueueViz step={current} />}
            {current && algorithm.kind === 'hashtable' && <HashTableViz step={current} />}
            {current && algorithm.kind === 'heap' && <HeapViz step={current} />}
            {current && algorithm.kind === 'trie' && <TrieViz step={current} />}
            {current && algorithm.kind === 'unionfind' && <UnionFindViz step={current} />}
            {current && algorithm.kind === 'segtree' && <SegTreeViz step={current} />}
          </div>
          {current?.message && (
