...
import type { BacktrackingSetAlgorithm } from '../types'
import { useStepPlayer } from '../hooks/useStepPlayer'
import { PlaybackControls } from './PlaybackControls'
import { AlgorithmCodePane } from './AlgorithmCodePane'
import { BacktrackingSetViz } from './BacktrackingSetViz'
import { PanelLeftClose, PanelLeftOpen, RotateCcw } from 'lucide-react'

type Props = { algorithm: BacktrackingSetAlgorithm }

export function BacktrackingSetVisualizer({ algorithm }: Props) {
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-hidden">
            {current && <BacktrackingSetViz step={current} />}
          </div>
          {current?.message && (
