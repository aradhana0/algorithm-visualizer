...
import type { LinkedListAlgorithm } from '../types'
...
export function LinkedListVisualizer({ algorithm }: Props) {
  const [seed, setSeed] = useState(0)
  const [codeCollapsed, setCodeCollapsed] = useState(false)
  const steps = useMemo(() => algorithm.generate(), [algorithm, seed])
  const player = useStepPlayer(steps, 500)
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-hidden">
            {current && <LinkedListViz step={current} />}
          </div>
          {current?.message && (
