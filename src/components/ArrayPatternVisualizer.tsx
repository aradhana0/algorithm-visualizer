...
import type { ArrayPatternAlgorithm } from '../types'
import { useStepPlayer } from '../hooks/useStepPlayer'
import { PlaybackControls } from './PlaybackControls'
import { AlgorithmCodePane } from './AlgorithmCodePane'
import { ArrayPatternViz } from './ArrayPatternViz'
import { PanelLeftClose, PanelLeftOpen, RotateCcw } from 'lucide-react'

type Props = { algorithm: ArrayPatternAlgorithm }

export function ArrayPatternVisualizer({ algorithm }: Props) {
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-hidden">
            {current && <ArrayPatternViz step={current} />}
          </div>
          {current?.message && (
